...
/**
 * Expand simple glob patterns (e.g., `packages/*`) into directories containing package.json.
 */
function expandWorkspaceGlob(rootDir: string, pattern: string): string[] {
	// Strip trailing glob stars: "packages/*" -> "packages", "apps/**" -> "apps"
...
/**
 * Detect workspace package directories from pnpm-workspace.yaml or package.json workspaces.
 * Returns absolute paths to directories that contain a package.json.
 */
function detectWorkspacePackages(rootDir: string): string[] {
	...
	if (existsSync(pnpmWorkspacePath)) {
		try {
			...
			if (Array.isArray(parsed?.packages)) {
				return parsed.packages.flatMap((pattern) => expandWorkspaceGlob(rootDir, pattern));
			}
		} catch {
			// Fall through
		}
	...
	if (existsSync(rootPkgPath)) {
		try {
			const pkg = JSON.parse(readFileSync(rootPkgPath, "utf-8")) as { workspaces?: string[] | { packages?: string[] } };
			const patterns = Array.isArray(pkg.workspaces) ? pkg.workspaces : pkg.workspaces?.packages;
			if (Array.isArray(patterns)) {
				return patterns.flatMap((pattern) => expandWorkspaceGlob(rootDir, pattern));
			}
		} catch {
			// Fall through
		}
...
export class TsDocResolver {
	...
	resolve(): TsDocResolverResult {
		...
		// Detect workspace packages from pnpm-workspace.yaml or package.json workspaces
